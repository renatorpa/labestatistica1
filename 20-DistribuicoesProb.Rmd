...
points(posicoes_barra, teorico$y,
       col = "magenta",
       pch = 19)
...
**7.** Um sistema de autenticação biométrica permite tentativas consecutivas até que o utilizador seja reconhecido com sucesso. Cada tentativa tem probabilidade de sucesso igual a 0.2, e o número de tentativas até o primeiro sucesso segue uma distribuição geométrica.
...
(c) Produza um histograma dos tempos simulados.

(d) Calcule a média e o desvio padrão de $T$.

(e) Estime a probabilidade empírica de que um acesso demore mais de 150 ms.

**8.** Um laboratório está a testar um novo medicamento que, em ensaios pré‑clínicos, elimina uma célula-alvo com probabilidade 0.1 por tentativa. O número de falhas antes do primeiro sucesso segue uma distribuição geométrica.
...
(a) Simule 5000 ensaios, cada um com $X \sim \text{Geom}(0.1)$.

(b) Calcule o vetor de custos $C$.

(c) Construa um boxplot dos custos simulados.
...
**9.** Um sistema de alta disponibilidade é composto por vários servidores redundantes. A cada tentativa de conexão, o sistema procura por um servidor funcional. Cada servidor tem 10% de chance de estar operacional. As tentativas são feitas sequencialmente até encontrar um servidor disponível. O número de servidores verificados até o primeiro sucesso segue uma distribuição geométrica com $p = 0.1$.
...
(b) Calcule o tempo de resposta $T$ para cada valor simulado.

(c) Crie um gráfico de dispersão entre $X$ e $T$.

(d) Calcule a média, mediana e desvio padrão de $T$.

(e) Estime a probabilidade empírica de o tempo ultrapassar 10 ms.

(f) Calcule os quantis de 25%, 50% e 75% de $T$.
