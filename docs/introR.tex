...
\begin{Highlighting}[]
\CommentTok{\# Sintaxe}
\FunctionTok{gl}\NormalTok{(n, k, }\AttributeTok{length =}\NormalTok{ n}\SpecialCharTok{*}\NormalTok{k, }\AttributeTok{labels =} \FunctionTok{seq\_len}\NormalTok{(n), }\AttributeTok{ordered =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  \texttt{length}: um inteiro que fornece o comprimento do resultado.
\item
...
\textbf{5.} Crie um factor representando níveis de escolaridade com os valores ``ensino secundário'', ``ensino superior'', ``mestrado'', ``doutoramento''. Defina uma ordem lógica para os níveis e ordene o factor em ordem crescente.
