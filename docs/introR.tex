...
\tightlist
\item
  \textbf{Teste 1} - Dia 28/10/25 (17h30-19h00) sala 8.2.39
\item
...
\NormalTok{minha\_lista[}\DecValTok{1}\NormalTok{] }\CommentTok{\# retorna uma sub{-}lista}
\DocumentationTok{\#\# $nome}
\DocumentationTok{\#\# [1] "Estudante"}

\CommentTok{\# Aceder a um elemento pelo índice (retorna o valor (vetor, número, etc.))}
\NormalTok{minha\_lista[[}\DecValTok{1}\NormalTok{]] }
\DocumentationTok{\#\# [1] "Estudante"}

\CommentTok{\# Aceder a um elemento pelo nome usando $ (retorna o valor do elemento com nome "nome")}
...
\NormalTok{minha\_lista[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{2}\NormalTok{]}
\DocumentationTok{\#\# $nome}
\DocumentationTok{\#\# [1] "Estudante"}
\DocumentationTok{\#\# }
\DocumentationTok{\#\# $idade}
...
\CommentTok{\# Adicionar novo elemento à lista}
...
\CommentTok{\# ou}
\NormalTok{minha\_lista[[}\StringTok{"nota\_final"}\NormalTok{]] }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(minha\_lista}\SpecialCharTok{$}\NormalTok{notas)}

\CommentTok{\# Remover um elemento da lista}
\NormalTok{minha\_lista}\SpecialCharTok{$}\NormalTok{idade }\OtherTok{\textless{}{-}} \ConstantTok{NULL}

\CommentTok{\# Remover elemento 2 e 3 da lista}
\NormalTok{minha\_lista[}\SpecialCharTok{{-}}\FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{)]}
\end{Highlighting}
\end{Shaded}
...
\textbf{1.} Um instituto de climatologia registou temperaturas mínimas e máximas durante 3 dias. Os dados estão no ficheiro \href{docs/temperaturas.txt}{Descarregar dados} com separador de tabulação e cabeçalho.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Use \texttt{read.table()} para ler ``temperaturas.txt''.
\item
  Visualize as 6 primeiras linhas com \texttt{head()}.
\item
  Qual foi a temperatura máxima no segundo dia?
\item
  Cria uma nova coluna \texttt{diferenca\_temp} com a diferença entre a temperatura máxima e a mínima. Salve o resultado num ficheiro \texttt{temperaturas\_nova.txt}. Dica: use \texttt{write.table(x=\ ,\ file=\ ,\ row.names=\ )}.
\end{enumerate}

\textbf{2.} Um ficheiro .csv exportado de um sistema português contém despesas e receitas mensais \href{docs/financas.csv}{Descarregar dados}. Os valores usam vírgula como separador decimal e ponto e vírgula como separador de campos.
...
\item
  Salve o resultado num ficheiro \texttt{financas\_nova.csv}. Dica: use \texttt{write.csv(x=\ ,\ file=\ ,\ row.names=\ )}.
\end{enumerate}

\textbf{3.} Um laboratório gerou um ficheiro sem cabeçalho com 4 colunas: experimento, tempo, concentração e erro. Os dados estão separados por espaços. \href{docs/resultados_lab.txt}{Descarregar dados}
...
\item
  Atribua nomes às colunas: ``experimento'', ``tempo'', ``concentracao'', ``erro''.
\item
  Qual a concentração do 5.º experimento?
\end{enumerate}

\textbf{4.} Um ficheiro .csv contém uma entrada incorreta na coluna \texttt{salario} (``quatro mil''). Isso causa erro ao tentar interpretar a coluna como numérica. \href{docs/dados_com_erros.csv}{Descarregar dados}

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Importe com \texttt{read.csv()} e verifique a classe da coluna salario.
\item
  Corrija os dados manualmente e converta salario em numeric.
\end{enumerate}

\textbf{5.} Faça a leitura do ficheiro \texttt{dados\_instagram} \href{https://renatorpaula.wixsite.com/renato/laboratorio-estatistica}{aqui} e responda às perguntas abaixo (lembre de instalar o pacote \texttt{readxl}):
...
\setcounter{enumi}{2}
\item
  \textbf{Visualização gráfica}: Crie representações gráficas adequadas para cada variável. Considere o uso de histogramas \texttt{hist()}, gráficos de barras \texttt{barplot()}, boxplots \texttt{boxplot()}, conforme o tipo de variável.
\item
...
\item
  \textbf{Correlação entre ``índice de influência'' e ``média de likes''}: Calcule a correlação entre as variáveis ``índice de influência'' e ``média de likes''. Comente sobre a relação entre essas duas variáveis, discutindo se um alto índice de influência tende a estar associado a uma média maior de curtidas. Dica: use \texttt{cor()}.
\end{enumerate}

\textbf{6.} Importe o ficheiro \texttt{dataset.csv} \href{https://renatorpaula.wixsite.com/renato/laboratorio-estatistica}{aqui} no R e salve-o como um data frame. Em seguida:
...
Reescreva o código acima usando o operador pipe (\texttt{\%\textgreater{}\%}). Dica: use a função \texttt{summarise()} para calcular a média.
...
Reescreva o código acima usando o operador pipe (\texttt{\%\textgreater{}\%}). Dica: use a função \texttt{mutate()} para criar a nova coluna.
...
\textbf{1} Um pesquisador está a analisar os resultados de uma pesquisa que avalia a satisfação de clientes com três diferentes serviços (A, B e C). Abaixo estão os dados recolhidos, em que cada linha representa um cliente e cada coluna indica a pontuação (de 1 a 10) atribuída a cada serviço:
...
\def\labelenumi{(\alph{enumi})}
\item
  Escreva uma função chamada \texttt{calcula\_media()} que receba o data frame \texttt{satisfacao} e devolva um vetor com a média de satisfação de cada cliente (média das 3 colunas).
\item
  Adicione ao data frame uma nova coluna chamada \texttt{media}, contendo os valores calculados pela função.
\item
  Com base na coluna \texttt{media}, use um loop \texttt{for} com estrutura \texttt{if-else} para classificar cada cliente como:
\end{enumerate}
...
Adicione uma nova coluna chamada \texttt{classificacao} ao data frame com essa informação.

\textbf{2.} Uma empresa registou o número de tarefas concluídas por 15 funcionários ao longo de 5 dias úteis. A meta da empresa é que cada funcionário realize pelo menos 20 tarefas por dia. Pretende-se agora verificar quem atingiu a meta em pelo menos 3 dos 5 dias.

Os dados estão organizados no seguinte data.frame, chamado \texttt{tarefa\_funcionarios}, onde cada linha representa um funcionário e cada coluna um dia da semana:
...
\def\labelenumi{(\alph{enumi})}
\item
  Escreva uma função chamada \texttt{verifica\_meta()} que receba o data.frame e devolva um vetor lógico indicando, para cada funcionário, se ele atingiu a meta em pelo menos 3 dias.
\item
  Com base no vetor lógico obtido, use um \texttt{for} com \texttt{if-else} para criar um vetor chamado \texttt{classificacao} com os valores:
\end{enumerate}
...
\tightlist
\item
  Adicione o vetor \texttt{classificacao} como uma nova coluna do data.frame \texttt{tarefa\_funcionarios}.
\end{enumerate}

\textbf{3.} Um estatístico está a estudar o comportamento da média amostral em diferentes populações. Para isso, simulou 6 amostras de tamanho 15 (valores entre 0 e 100), e pretende calcular a média de cada uma. A simulação deve interromper-se assim que for encontrada uma média superior a 75.

Os dados encontram-se na seguinte matriz chamada \texttt{amostras}:
...
\def\labelenumi{(\alph{enumi})}
\item
  Crie uma função chamada \texttt{calcula\_media\_amostra()} que receba a matriz amostras e devolva a média de uma linha específica (dada por um índice).
\item
  Escreva um ciclo \texttt{while} que percorra as linhas da matriz, calculando a média de cada amostra uma a uma. Se a média for superior a 75, o programa deve imprimir a mensagem: ``Média Excedeu Limite'' e parar a execução. Caso contrário, o programa deve continuar até processar todas as amostras.
\end{enumerate}

\textbf{4.} Crie uma lista chamada \texttt{listas\_transformadas} com comprimento 7, onde o elemento de índice \(i\) deve conter a seguinte sequência:
...
\def\labelenumi{(\alph{enumi})}
\item
  Crie a lista \texttt{listas\_transformadas} conforme descrito, utilizando um loop \texttt{for}.
\item
  Verifique o comprimento do último vetor da lista.
\item
  Escreva uma função chamada \texttt{soma\_elementos()} que receba uma lista como entrada e devolva um vetor com a soma de cada vetor contido na lista.
\item
  Use a função \texttt{soma\_elementos()} para calcular a soma dos elementos de cada vetor da lista \texttt{listas\_transformadas}.
\end{enumerate}

\textbf{5.} Crie dois vetores \(x\) e \(y\), ambos com comprimento 60, cujos valores devem ser inteiros aleatórios escolhidos com repetição entre 0 e 999 (inclusive).

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Gere os vetores \(x\) e \(y\) conforme descrito. Utilize \texttt{sample()} com \texttt{replace\ =\ TRUE}.
\item
  Selecione os valores de \(y\) que são menores que 500 e cujos correspondentes em \(x\) (mesma posição) são múltiplos de 3.
\item
  Crie um novo vetor chamado \texttt{dif\_absoluta}, contendo o valor absoluto da diferença entre \(x\) e \(y\) (isto é, \(|x_i - y_i|\) para cada posição \(i\)).
\item
  Calcule a média e o desvio padrão dos valores em \texttt{dif\_absoluta}.
\end{enumerate}

\textbf{6.} Seja \(x\) um vetor de números reais aleatórios com comprimento 80, gerado a partir de uma distribuição normal com média 5 e desvio padrão 2.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Gere o vetor \(x\) com 80 observações da distribuição \(\mathcal{N}(5, 2)\), utilizando a função \texttt{rnorm()}.
\item
  Calcule a média amostral \(\bar{x}\) do vetor \(x\).
\item
  Crie um novo vetor \texttt{resultado}, onde o i-ésimo elemento é definido por:
\end{enumerate}
...
\tightlist
\item
  Calcule o valor máximo, mínimo e a média do vetor \texttt{resultado}.
\end{enumerate}

\textbf{7.} Um climatologista está a estudar as temperaturas médias diárias em 12 cidades ao longo de uma semana (7 dias). O seu objetivo é calcular a temperatura média semanal de cada cidade e verificar se essa média está acima ou abaixo de uma temperatura de referência de 25ºC.

Os dados abaixo representam as temperaturas médias diárias (em ºC) registadas durante uma semana em cada cidade. Estes dados estão organizados num data.frame chamado \texttt{temperaturas\_cidades}, onde:
...
\def\labelenumi{(\alph{enumi})}
\item
  Escreva uma função chamada \texttt{calcula\_media\_temperaturas()} que receba o data.frame \texttt{temperaturas\_cidades} e devolva um vetor com a temperatura média semanal de cada cidade.
\item
  Crie um vetor chamado \texttt{classificacao}, utilizando um ciclo \texttt{while}, que percorra as médias semanais e, com base numa comparação com o valor de referência de 25ºC, classifique cada cidade como:
\end{enumerate}
...
\tightlist
\item
  Adicione os vetores \texttt{media\_semanal} e \texttt{classificacao} como novas colunas no data.frame \texttt{temperaturas\_cidades}.
\end{enumerate}

\textbf{8.} Os computadores podem sofrer alterações aleatórias em bits da memória (bit flips) devido à ação de partículas de alta energia, como os raios gama. Esse fenómeno é estudado em contextos críticos como aviação, medicina, criptografia e computação de alto desempenho.
...
\CommentTok{\# Vetor que representa o estado original de 1000 bits (0 ou 1)}
...
\CommentTok{\# Vetor que representa o estado final após exposição à radiação (raios gama)}
\CommentTok{\# Cada bit tem 3\% de probabilidade de ser alterado (flipado)}
...
\def\labelenumi{(\alph{enumi})}
\item
  Crie um vetor \texttt{erros} que contenha o número de bits alterados (bit flips), comparando original com \texttt{flipado}.
\item
  Crie um fator chamado \texttt{bloco\_memoria} com os valores de 1 a 10, indicando o bloco de cada bit, como já feito na variável \texttt{blocos}.
\item
  Calcule o número de bit flips em cada bloco de memória.
\end{enumerate}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-181-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-182-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-183-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-184-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-185-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-187-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-189-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-191-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-192-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-194-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-195-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-196-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-198-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-199-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-201-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-202-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-203-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-205-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-243-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-244-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-245-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-246-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-247-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-248-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-249-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-257-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-262-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-263-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-269-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-278-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-281-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-282-1.pdf}}
...
\FunctionTok{points}\NormalTok{(posicoes\_barra, teorico}\SpecialCharTok{$}\NormalTok{y,}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-283-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-284-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-285-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-285-2.pdf}}
...
\textbf{7.} Um sistema de autenticação biométrica permite tentativas consecutivas até que o utilizador seja reconhecido com sucesso. Cada tentativa tem probabilidade de sucesso igual a 0.2, e o número de tentativas até o primeiro sucesso segue uma distribuição geométrica.

Um engenheiro pretende simular 1000 acessos diferentes ao sistema, assumindo que o tempo total de processamento associado ao número de tentativas \(X\) é dado pela função polinomial:

\[T = 2X^2 + 3X + 5,\]

onde \(T\) representa o tempo em milissegundos.
...
\item
  Produza um histograma dos tempos simulados.
\item
  Calcule a média e o desvio padrão de \(T\).
\item
  Estime a probabilidade empírica de que um acesso demore mais de 150 ms.
\end{enumerate}

\textbf{8.} Um laboratório está a testar um novo medicamento que, em ensaios pré‑clínicos, elimina uma célula-alvo com probabilidade 0.1 por tentativa. O número de falhas antes do primeiro sucesso segue uma distribuição geométrica.

O custo associado a um ensaio é modelado pela função polinomial:

\[C = 0.5X^2 + 20,\]

onde \(X\) é o número de falhas antes do sucesso e \(C\) é o custo em euros.

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  Simule 5000 ensaios, cada um com \(X \sim \text{Geom}(0.1)\).
\item
  Calcule o vetor de custos \(C\).
\item
  Construa um boxplot dos custos simulados.
\item
  Calcule:
\end{enumerate}
...
\item
  O desvio padrão
\item
  A percentagem de ensaios com custo superior a 200€
\item
  A percentagem com custo inferior a 50€
\end{itemize}

\textbf{9.} Um sistema de alta disponibilidade é composto por vários servidores redundantes. A cada tentativa de conexão, o sistema procura por um servidor funcional. Cada servidor tem 10\% de chance de estar operacional. As tentativas são feitas sequencialmente até encontrar um servidor disponível. O número de servidores verificados até o primeiro sucesso segue uma distribuição geométrica com \(p = 0.1\).

O tempo total de resposta do sistema é modelado pela função:

\[T = \sqrt{X^2 + 3X + 1} + \log(X + 1),\]

onde \(X\) é o número de tentativas necessárias até encontrar um servidor ativo, e \(T\) é o tempo de resposta em milissegundos.
...
\item
  Calcule o tempo de resposta \(T\) para cada valor simulado.
\item
  Crie um gráfico de dispersão entre \(X\) e \(T\).
\item
  Calcule a média, mediana e desvio padrão de \(T\).
\item
  Estime a probabilidade empírica de o tempo ultrapassar 10 ms.
\item
  Calcule os quantis de 25\%, 50\% e 75\% de \(T\).
\end{enumerate}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-288-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-289-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-290-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-291-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-291-2.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-295-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-297-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-298-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-299-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-300-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-303-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-304-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-309-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-310-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-323-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-325-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-326-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-327-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-328-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-329-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{introR_files/figure-latex/unnamed-chunk-330-1.pdf}}
